...
def generar_licencia(email, empresa, tier, dias):
...
def enviar_email_licencia(email_destino, empresa, tier, lic_bytes, filename):
    msg = MIMEMultipart()
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            lic_bytes, filename = generar_licencia(customer_email, customer_name, tier, dias_validez)
        except Exception as e:
        ...
        if SMTP_PASS and customer_email:
            try:
                enviar_email_licencia(customer_email, customer_name, tier, lic_bytes, filename)
                email_sent = True
            except Exception as e:
